...
class ResidualBlock(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def call(self, inputs, training):
    ...
    for layer in self.shortcut_layers:
      ...
      if isinstance(layer, Conv2dFixedPadding):
        shortcut_feats.append(shortcut)

    conv2d_bn_feats = []
    for layer in self.conv2d_bn_layers:
      inputs = layer(inputs, training=training)
      if isinstance(layer, Conv2dFixedPadding):
        conv2d_bn_feats.append(inputs)

    if FLAGS.se_ratio > 0:
      inputs = self.se_layer(inputs, training=training)

    return tf.nn.relu(inputs + shortcut), shortcut_feats, conv2d_bn_feats
...
class BottleneckBlock(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training):
    ...
    return tf.nn.relu(inputs + shortcut), inputs + shortcut
...
class BlockGroup(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def call(self, inputs, training):
    ...
    for layer in self.layers:
      inputs, s_feat, conv_feat = layer(inputs, training=training)
      for f in conv_feat:
        conv2d_feats.append(f)
      for f in s_feat:
        short_feats.append(f)
    assert(len(short_feats) == 1)
    feats = []
    for f in conv2d_feats:
      if FLAGS.sep_short_direct_branch:
        temp_feat = []
        temp_feat.append(f)
        temp_feat.append(short_feats[0])
        feats.append(temp_feat)
      else:
        feats.append(tf.concat([f, short_feats[0]], axis=1))
    return tf.identity(inputs, self._name), feats
...
class Resnet(tf.keras.layers.Layer):  # pylint: disable=missing-docstring
  ...
  def call(self, inputs, training):
    ...
    for layer in self.initial_conv_relu_max_pool:
      inputs = layer(inputs, training=training)
      if layer.name == 'initial_conv':
        if FLAGS.use_GAP_HSIC_features:
          if self.data_format == 'channels_last':
            if curr_index in FLAGS.HSIC_feature_layers or FLAGS.use_all_features_HSIC:
              feats.append(tf.reduce_mean(inputs, [1, 2]))
          else:
            if curr_index in FLAGS.HSIC_feature_layers or FLAGS.use_all_features_HSIC:
              feats.append(tf.reduce_mean(inputs, [2, 3]))
        else:
          if curr_index in FLAGS.HSIC_feature_layers or FLAGS.use_all_features_HSIC:
            if FLAGS.use_random_projections:
              temp_input = tf.reshape(inputs, (tf.shape(inputs)[0], -1))
              # if tf.shape(temp_input)[1] > 2*FLAGS.random_proj_dim:
              ...
              # else:
              # feats.append(temp_input)
        curr_index += 1

    for i, layer in enumerate(self.block_groups):
      if FLAGS.train_mode == 'finetune' and FLAGS.fine_tune_after_block == i:
        inputs = tf.stop_gradient(inputs)
      inputs, curr_feat = layer(inputs, training=training)
      for j in range(len(curr_feat)):
        if FLAGS.use_GAP_HSIC_features:
          if self.data_format == 'channels_last':
            if curr_index in FLAGS.HSIC_feature_layers or FLAGS.use_all_features_HSIC:
              if FLAGS.sep_short_direct_branch:
                for k in range(len(curr_feat[j])):
                  feats.append(tf.reduce_mean(curr_feat[j][k], [1, 2]))
              else:
                feats.append(tf.reduce_mean(curr_feat[j], [1, 2]))
          else:
            if curr_index in FLAGS.HSIC_feature_layers or FLAGS.use_all_features_HSIC:
              if FLAGS.sep_short_direct_branch:
                for k in range(len(curr_feat[j])):
                  feats.append(tf.reduce_mean(curr_feat[j][k], [2, 3]))
              else:
                feats.append(tf.reduce_mean(curr_feat[j], [2, 3]))
        else:
          if curr_index in FLAGS.HSIC_feature_layers or FLAGS.use_all_features_HSIC:
            if FLAGS.use_random_projections:
              temp_input = tf.reshape(inputs, (tf.shape(inputs)[0], -1))
