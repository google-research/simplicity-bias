...
def generate_ub_linslab_data_diffmargin_v2(N_tr, dim, eff_lin_margins, eff_slab_margins,
    ...
    for idx, coord in enumerate(range(i, i+num_slabs)):
        ...
        if slab_per == 5:
            ...
            if total_corrupt > 0:
                if corrupt_5slab_margin:
                    ...
                    offset = (0.5*slab_width+2*slab_margin)
                    X[idx_n, coord] = get_sign(len(idx_n))*(offset+slab_width*get_unif(len(idx_n)))
                    ...
                    X[idx_p_ctr, coord] = 0.5*get_sign(len(idx_p_ctr))*slab_width*get_unif(len(idx_p_ctr))

                    # move negative points to positve slabs
                    offset = 1.5*slab_width + 4*slab_margin
                    X[idx_p_sid, coord] = get_sign(len(idx_p_sid))*(offset+slab_width*get_unif(len(idx_p_sid)))
        ...
        if slab_per == 7:
            ...
            print ("total corrupt: {}".format(total_corrupt))
            if total_corrupt > 0:
                # corrupt data
                ...
                offset = 0.5*slab_width+2*slab_margin
                X[idx_n_inner, coord] = get_sign(len(idx_n_inner))*(offset+slab_width*get_unif(len(idx_n_inner)))
                offset = 2.5*slab_width+6*slab_margin
                X[idx_n_outer, coord] = get_sign(len(idx_n_outer))*(offset+slab_width*get_unif(len(idx_n_outer)))
